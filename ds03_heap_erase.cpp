...
namespace CP {
  ...
  public:
    ...
    void erase(const T& v) {
      ...
      mData[dataToEraseIndex] = mData[mSize - 1];
      mSize--;

      if (mLess(mData[(dataToEraseIndex - 1) / 2], mData[dataToEraseIndex])) //max
      {
        fixUp(dataToEraseIndex);
      }
      else
      {
        fixDown(dataToEraseIndex);
      }
    }
