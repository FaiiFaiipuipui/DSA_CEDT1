...
void node_generator(int p, int n, int h, CP::map_bst<int,int>& bst, int cutValue)
{
  if (n == h) return;
  bst[p] = p;
  h++;
  cutValue /= 2;
  node_generator(p - cutValue, n, h, bst, cutValue);
  node_generator(p + cutValue, n, h, bst, cutValue);
}
...
void gen_best_bst(int n, CP::map_bst<int, int>& bst) {
  ...
  node_generator((n / 2) + 1, ogInput, 0, bst, (n / 2) + 1);
  return;
}
