...
class list
{
  ...
  public:
    ...
    void reorder(int pos,std::vector<int> selected) { // WRITE HERE
      node* tmp = mHeader->next;
      std::vector <node*> selected_node;
      // save selected node*
      ...
      // save pos node*
      node* postiton = mHeader->next;
      for (int i = 0; i < pos; i++)
      {
        postiton = postiton->next;
      }

      // connect prev and next of selected node;
      for (int i = 0; i < selected.size(); i++)
      {
        (selected_node.at(i)->prev)->next = selected_node.at(i)->next;
        (selected_node.at(i)->next)->prev = selected_node.at(i)->prev;
      }

      // connect selected node;
      for (int i = 0; i < selected.size() - 1; i++)
      {
        selected_node.at(i)->next = selected_node.at(i + 1);
        selected_node.at(i + 1)->prev = selected_node.at(i);
      }
      // now only begin prev & rbegin prev of selected_node is not set
      // WRONG
      selected_node.at(0)->prev = postiton->prev;
      (postiton->prev)->next = selected_node.at(0);
      selected_node.at(selected_node.size() - 1)->next = postiton;
      postiton->prev = selected_node.at(selected_node.size() - 1);
